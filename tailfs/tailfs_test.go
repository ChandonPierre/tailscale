...
const (
	...
	remote1 = "remote1"
	remote2 = "_remote2"
	// TODO(oxtoacart): try some more complicated names, including illegal characters like slashes
	share11 = "share11"
	share12 = "_share12"
	file111 = "file111.txt"
)
...
func (s *system) addRemote(name string) {
	...
	remotes := make(map[string]string, len(s.remotes))
	for name, r := range s.remotes {
		remotes[name] = fmt.Sprintf("http://%s", r.l.Addr())
	}
