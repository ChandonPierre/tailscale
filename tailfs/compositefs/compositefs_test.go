...
func TestStatwithStatChildren(t *testing.T) {
	cfs, dir1, dir2, _, close := createFileSystem(t, &Opts{StatChildren: true})
	defer close()
...
func createFileSystem(t *testing.T, opts *Opts) (webdav.FileSystem, string, string, *tstest.Clock, func()) {
	...
	fs := New(opts)
	fs.AddChild("remote4", &closeableFS{webdav.Dir(dir2)})
	fs.SetChildren(map[string]webdav.FileSystem{
		"remote2": webdav.Dir(dir2),
		"remote3": &closeableFS{webdav.Dir(dir2)},
	})
	fs.AddChild("remote1", webdav.Dir(dir1))
